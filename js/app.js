class SkorpikValueApp {
    ...
    // Добавляем в метод addItemToCalculator
addItemToCalculator(panel, itemData) {
    ...
    if (existingItem) {
        ...
        removeBtn.addEventListener('click', (e) => {
            ...
            this.updateSectionValue(panel); // Обновляем Value при удалении
        });
    ...
    this.updateCalculatorResults();
    this.repositionAddButton(container);
    this.updateSectionValue(panel); // Обновляем Value при добавлении
}

// Добавляем новый метод для обновления Value
updateSectionValue(panel) {
    ...
    // Создаем или обновляем элемент Value
